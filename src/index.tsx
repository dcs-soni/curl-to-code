...
async function main() {
  ...
  const argv = await yargs(hideBin(process.argv))
    ...
    .option("clipboard", {
      type: "boolean",
      default: false,
      description: "Copy generated code to system clipboard",
    })
    .help()
    .alias("help", "h")
    .version()
  ...
  try {
    ...
    if (argv.output || argv.clipboard) {
      ...
      if (argv.output) {
        ...
        note(chalk.green(`Successfully wrote API types to ${safeOutputPath}`), "File Output");
      }

      if (argv.clipboard) {
        await clipboardy.write(formatted);
        note(chalk.green("Generated code copied to clipboard!"), "Clipboard");
      }
    }
